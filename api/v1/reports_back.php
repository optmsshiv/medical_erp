...
// Loose Sale: also pull loose_qty + sub_unit so the Reports page can show a
// "Loose Stock" figure alongside sealed-pack qty (batches.quantity is unchanged
// in meaning — still sealed packs only).
$stmt = $pdo->query(
    "SELECT b.*, m.name AS med_name, m.min_stock, m.sub_unit
...
$stock = array_map(function ($r) {
    ...
    return [
        ...
        'looseQty' => (int) $r['loose_qty'], 'subUnit' => $r['sub_unit'] ?? '',

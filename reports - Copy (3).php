        ...
        <div class="tab-content">
          ...
          <div class="tab-pane fade" id="rp-stock">
            <div class="card-mf">
              ...
              <div class="table-scroll" style="max-height:none">
                <table class="table table-mf">
                  <thead><tr><th>Medicine</th><th>Batch</th><th class="text-end">Qty</th><th class="text-end">Loose Stock</th><th class="text-end">Purchase Value</th><th class="text-end">MRP Value</th><th>Status</th></tr></thead>
    ...
    await (async function () {
      ...
      function renderStock() {
        ...
        $('#rpStockBody').innerHTML = rows.map((b) => `
          ...
          </tr>`).join('') || `<tr><td colspan="7"><div class="empty-state"><i class="bi bi-box-seam"></i>No stock recorded.</div></td></tr>`;
        $('#rpStockFoot').innerHTML = rows.length ? `<tr><td colspan="2">Total</td><td class="text-end num">${MF.num(rows.reduce((s, b) => s + b.qty, 0))}</td><td></td><td class="text-end num">${MF.fmt(rows.reduce((s, b) => s + b.purchaseValue, 0))}</td><td class="text-end num">${MF.fmt(rows.reduce((s, b) => s + b.mrpValue, 0))}</td><td></td></tr>` : '';
      }
